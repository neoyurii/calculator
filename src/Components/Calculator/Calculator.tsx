...
export default function Calculator() {
  ...
  return (
    <section
      className="mw-100 p-5 bg-info-subtle rounded"
      style={{ width: "min(80%, 425px)" }}
    >
      <form
      ...
      >
        {inputs.map((item) => {
          return (
            <div
              key={item.id}
              className="input-group bg-light rounded overflow-hidden"
            >
              <label
                htmlFor={item.id}
                className="w-100 p-1 bg-info text-white fs-5 fw-medium"
              >
                {item.label}
              </label>
              <input
        ...
        <button
          type="submit"
          className="w-100 btn btn-info btn-lg text-dark"
          disabled={loading}
        >
          Calcular
        </button>
